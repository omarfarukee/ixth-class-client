...
export default function EditTeacherImage() {
    ...
    const handleAddImage = async (data) => {
        ...
        try {
          ...
          if (imgData.success) {
            ...
            const updateResponse = await fetch(`http://localhost:5000/update/teacher/${studentData?._id}`, {
              method: 'PUT',
              headers: {
            ...
            const teacherProfileImage = await updateResponse.json();
            console.log(teacherProfileImage)
            if (teacherProfileImage?.result?.modifiedCount === 1) {
    
              const updatedTeacherData = { ...studentData,image: imgData.data.url };
